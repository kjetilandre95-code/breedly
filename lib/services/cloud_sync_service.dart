...
class FirestoreService {
  ...
  /// (data belongs to the user's private profile). New root records must carry
  /// an explicit active tombstone value because reads filter on isDeleted=false.
  ...
  Future<RepositoryWriteResult> migrateToFlatCollections(String userId) async {
    try {
      ...
      debugPrint(
          '[MIGRATE] Done: $dogsMigrated dogs, $littersMigrated litters, $showResultsMigrated show results migrated to flat collections.');
      return RepositoryWriteResult.success();
    } catch (e) {
